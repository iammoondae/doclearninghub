...
let REMOTE_MANIFEST_URL = localStorage.getItem('remote_manifest_url') || 'https://raw.githubusercontent.com/iammoondae/doclearninghub/main/lecturenotes/manifest.json';
...
function onboardingGoogleLogin() {
  // Input Google account
  const emailInput = prompt("Enter your academic Google Email (must end with @msugensan.edu.ph):", "juan.delacruz@msugensan.edu.ph");
  ...
  // Pre-fill student name from email parts
  const localPart = email.split('@')[0];
  const nameParts = localPart.split('.').map(word => word.charAt(0).toUpperCase() + word.slice(1));
  const mockName = nameParts.join(' ');
  ...
  document.getElementById('onboarding-nickname').value = currentUser.name;
}
...
function renderSyllabusView() {
  ...
  const planRows = (details.instructionalPlan || []).map(row => {
    return `
      <tr>
        ...
        <td style="font-weight:600; color:var(--accent);">${row.outcome}</td>
  ...
  viewport.innerHTML = `
    <div class="syllabus-view-container" style="animation: fadeIn 0.3s ease;">
      ...
      <div class="section-card" style="margin-bottom:24px; padding:20px; border-radius:12px; background:var(--bg-card); border:1px solid var(--border-card);">
        <h3 style="margin-top:0; margin-bottom:4px; font-family:'Outfit',sans-serif;">Instructional Plan</h3>
        <p style="font-size:12px; color:var(--text-muted); margin-top:0; margin-bottom:14px;">Verbatim schedule and outline of course modules.</p>

        <!-- Zoom Controls -->
        <div class="table-zoom-controls">
          <span class="table-zoom-label">Table Zoom:</span>
          <button class="zoom-btn" onclick="zoomTable(-0.1)">➖ Zoom Out</button>
          <button class="zoom-btn" onclick="zoomTable(0.1)">➕ Zoom In</button>
          <button class="zoom-btn" onclick="resetTableZoom()">🔄 Reset</button>
        </div>
          ...
          <table class="zoomable-table">
            <thead>
              <tr>
                ...
                <th style="width:25%;">Topic / Content</th>
                <th style="width:40%;">Flexible Teaching-Learning Strategies</th>
                <th style="width:20%;">Assessment</th>
              </tr>
            </thead>
            <tbody>
              ${planRows || '<tr><td colspan="5" style="text-align:center;">No plan details available.</td></tr>'}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div class="module-card" style="margin-bottom: 30px; border-left: 4px solid var(--accent); border-radius: 12px; padding: 18px;">
        ...
        <div class="module-actions" style="margin-top: 10px;">
          <button class="pdf-action-btn" style="border-radius: 8px; padding: 8px 16px;" onclick="viewSyllabusInApp('${activeCourse.syllabus?.pdfUrl}', '${activeCourse.syllabus?.title}')">👁️ View In App</button>
          <button class="pdf-action-btn" style="background:#475569; border-radius: 8px; padding: 8px 16px;" onclick="exportPDFExternally('${activeCourse.syllabus?.pdfUrl}')">📤 Export File</button>
        </div>
      </div>
    ...
    <div class="syllabus-view-container" style="animation: fadeIn 0.3s ease;">
      ...
      <div class="foundation-section">
        ...
        <div class="foundation-card eoms-card">
          ...
          <p class="foundation-text">${foundations.eoms}</p>
        </div>
...
function renderGuidelinesView() {
  ...
  viewport.innerHTML = `
    <div class="syllabus-view-container" style="animation: fadeIn 0.3s ease;">
      <h2>💡 Guidelines & References</h2>
      <p style="margin-top:-10px; color:var(--text-muted); font-size: 14px; margin-bottom: 25px;">
        References, guidelines for study optimization, and instructor contact details.
      </p>

      <!-- Faculty Info -->
      <div class="section-card" style="margin-bottom:24px; padding:20px; border-radius:12px; background:var(--bg-card); border:1px solid var(--border-card);">
        <h3 style="margin-top:0; margin-bottom:14px; font-family:'Outfit',sans-serif;">👨‍🏫 Faculty Information</h3>
        ${facultyHTML}
      </div>

      <!-- Guidelines -->
      <div class="section-card" style="margin-bottom:24px; padding:20px; border-radius:12px; background:var(--bg-card); border:1px solid var(--border-card);">
        <h3 style="margin-top:0; margin-bottom:14px; font-family:'Outfit',sans-serif;">💡 Additional Guidelines & Recommendations</h3>
      ...
      <!-- References -->
      <div class="section-card" style="margin-bottom:24px; padding:20px; border-radius:12px; background:var(--bg-card); border:1px solid var(--border-card);">
        <h3 style="margin-top:0; margin-bottom:14px; font-family:'Outfit',sans-serif;">📚 Verbatim Syllabus References</h3>
...
function zoomTable(change) {
  tableZoomScale = Math.min(1.4, Math.max(0.7, tableZoomScale + change));
  const table = document.querySelector('.zoomable-table');
  if (table) {
    table.style.fontSize = `${tableZoomScale}em`;
  }
}
...
function viewSyllabusInApp(pdfUrl, title) {
  playSFX(true);
  const container = document.getElementById('pdf-syllabus-viewer-box');
  if (!container) return;

  const cleanUrl = encodeURIComponent(pdfUrl);
  const viewerIframeUrl = `https://mozilla.github.io/pdf.js/web/viewer.html?file=${cleanUrl}`;

  container.innerHTML = `
    <div class="pdf-viewer-container" style="margin-bottom: 25px;">
      <div class="pdf-viewer-header">
        <span class="pdf-viewer-title">📋 Reading Syllabus: ${title}</span>
        <div class="pdf-viewer-actions">
          <button class="pdf-action-btn" style="background:#64748b;" onclick="closeSyllabusPDF()">Close Reader &times;</button>
        </div>
      </div>
      <iframe class="pdf-iframe" src="${viewerIframeUrl}" title="In-App Syllabus Reader"></iframe>
    </div>
  `;
  ...
  activeCourse.modules.forEach(m => {
    html += `
      <div class="module-card" id="note-card-${m.id}">
        ...
        <div class="module-actions">
          <button class="pdf-action-btn" onclick="viewPDFInApp('${m.pdfUrl}', '${m.title}')">👁️ View In App</button>
          <button class="pdf-action-btn" style="background:#475569;" onclick="exportPDFExternally('${m.pdfUrl}')">📤 Export File</button>
        </div>
      </div>
    `;
  });
...
function viewPDFInApp(pdfUrl, title) {
  playSFX(true);
  const container = document.getElementById('pdf-inapp-viewer-box');
  if (!container) return;

  // Since Android system webviews lack native PDF capabilities, we load the PDF 
  // through the official PDF.js Mozilla viewer online iframe or standard viewer link
  const cleanUrl = encodeURIComponent(pdfUrl);
  const viewerIframeUrl = `https://mozilla.github.io/pdf.js/web/viewer.html?file=${cleanUrl}`;
  ...
  container.innerHTML = `
    <div class="pdf-viewer-container" style="margin-bottom: 25px;">
      <div class="pdf-viewer-header">
        <span class="pdf-viewer-title">📖 Reading: ${title} Handout</span>
        <div class="pdf-viewer-actions">
          <button class="pdf-action-btn" style="background:#64748b;" onclick="closeInAppPDF()">Close Reader &times;</button>
        </div>
      </div>
      <iframe class="pdf-iframe" src="${viewerIframeUrl}" title="In-App Handout Reader"></iframe>
    </div>
  `;
  container.style.display = 'block';
  container.scrollIntoView({ behavior: 'smooth' });
...
function exportPDFExternally(pdfUrl) {
  playSFX(true);
  // Opens in native browser or system PDF viewer
...
function renderAssessmentsView() {
  ...
  let html = `
    <h2>✍️ Quizzes & Performance Tasks</h2>
  ...
  activeCourse.modules.forEach(m => {
    ...
    if (m.quiz) {
      ...
      if (savedScore !== null) {
        ...
        quizSectionHTML = `
          ...
          <span style="font-size: 11px; color: var(--incorrect, #ef4444); display: block; text-align: center; margin-top: 6px; font-weight: 600;">⚠️ Retakes are strictly prohibited once submitted</span>
        `;
      }
    }

    let assignmentSectionHTML = '';
    // Only show if the assignment has a prepared pre-filled form (not the default placeholder URL ID)
    ...
    html += `
      <div class="module-card" id="assessment-card-${m.id}" style="flex-direction: column; align-items: stretch; gap: 10px;">
        <div class="module-info">
          <span class="module-title">${m.title} Tasks</span>
...
function openAssignmentForm(modId, formUrl) {
  ...
  // Submit optional Apps Script log ping first to record student action
...
function submitQuizResults() {
  ...
  // Submit record to Google Sheets in background
...
function renderStudentProgressView() {
  ...
  if (quizTableRows === '') {
    quizTableRows = `<tr><td colspan="4" style="text-align:center; padding:20px; color:var(--text-muted);">No quizzes available in syllabus.</td></tr>`;
  }
...
function signOutStudent() {
  playSFX(true);
  const confirmOut = confirm("Sign out student session? Scores history remains locally, but current profile details will be cleared.");
