...
function renderLabSettingsView() {
  ...
  viewport.innerHTML = `
    ...
    <div style="background:var(--bg-card); border:1px solid var(--border-card); border-radius:16px; padding:20px; text-align:left; margin-bottom:24px;">
    ...
    <!-- User Promotion tool -->
    <div style="background:var(--bg-card); border:1px solid var(--border-card); border-radius:16px; padding:20px; text-align:left;">
      <h3 style="margin:0 0 8px 0; font-size:15px; font-weight:700; font-family:'Outfit',sans-serif;">👥 User Roster Promotions</h3>
      <p style="margin:0 0 16px 0; font-size:12.5px; color:var(--text-muted);">Quickly lookup any registered system user and adjust their workspace permissions (Admin, Faculty, Laboratory Custodian, or Student).</p>
      
      <div style="display:flex; gap:10px; margin-bottom:18px; max-width:500px; align-items:flex-end;">
        <div style="display:flex; flex-direction:column; gap:4px; flex:1;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Search User Email:</label>
          <input type="text" id="lims-promote-search-email" placeholder="e.g. name@msugensan.edu.ph" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
        </div>
        <button class="settings-btn-primary" onclick="lookupUserForPromotion()" style="width:auto; margin:0; padding:10px 18px; font-size:13px; background:var(--accent); color:white;">🔍 Search</button>
      </div>

      <div id="lims-promotion-target-area"></div>
    </div>
  `;
}
...
function lookupUserForPromotion() {
  const email = document.getElementById('lims-promote-search-email').value.trim().toLowerCase();
  const container = document.getElementById('lims-promotion-target-area');
  ...
  container.innerHTML = `<div style="font-size:12.5px; color:var(--text-muted); font-style:italic;">Searching profile...</div>`;

  firestore.collection('students').doc(email).get()
    .then(doc => {
      if (!doc.exists) {
        container.innerHTML = `
          <div style="padding:12px; border:1px dashed var(--border-card); border-radius:8px; color:var(--text-muted); font-size:13px;">
            ⚠️ Profile for <strong>${escapeHtml(email)}</strong> does not exist in the students database yet. 
            They must complete onboarding sign-in once.
          </div>
        `;
        return;
      }
      ...
      container.innerHTML = `
        <div style="padding:16px; border:1px solid var(--border-card); border-radius:12px; background:var(--bg-body); display:flex; flex-direction:column; gap:12px;">
          <div>
            <div style="font-weight:700; font-size:14px; color:var(--text-main);">${escapeHtml(u.name || 'Anonymous')}</div>
            <div style="font-size:12px; color:var(--text-muted);">${escapeHtml(doc.id)}</div>
            <div style="font-size:12px; color:var(--text-muted); margin-top:2px;">Current Role: <strong style="text-transform:uppercase; color:var(--accent);">${escapeHtml(currentRole)}</strong></div>
          </div>
          
          <div style="display:flex; align-items:center; gap:10px; border-top:1px dashed var(--border-card); padding-top:12px; flex-wrap:wrap;">
            <label style="font-size:11.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Promote / Demote To:</label>
            <select id="lims-promotion-select" style="padding:8px; border-radius:6px; border:1px solid var(--border-card); background:var(--bg-card); color:var(--text-main); font-size:12.5px; font-weight:600; cursor:pointer;">
              <option value="student" ${currentRole === 'student' ? 'selected' : ''}>Student</option>
              <option value="faculty" ${currentRole === 'faculty' ? 'selected' : ''}>Faculty Instructor</option>
              <option value="laboratory" ${currentRole === 'laboratory' ? 'selected' : ''}>Lab Stockroom Custodian</option>
              <option value="admin" ${currentRole === 'admin' ? 'selected' : ''}>System Admin</option>
            </select>
            <button class="settings-btn-primary" onclick="confirmLimsPromotion('${escapeJsString(doc.id)}')" style="width:auto; margin:0; padding:8px 16px; font-size:12px; font-weight:600; background:#10b981; color:white;">💾 Save Role</button>
          </div>
        </div>
      `;
    })
    .catch(err => {
      container.innerHTML = `<div style="color:var(--incorrect);">Search Error: ${err.message}</div>`;
    });
}

function confirmLimsPromotion(email) {
  const newRole = document.getElementById('lims-promotion-select').value;
  if (!confirm(`Are you sure you want to assign the role of ${newRole.toUpperCase()} to ${email}?`)) return;

  updateUserRoleDatabase(email, newRole);
}
