...
// PCO EMIS LIMS Integration functions
function triggerLimsChemicalWasteForm() {
  firestore.collection('pco_inventory').where('status', '==', 'active').get().then(snap => {
    if (snap.empty) {
      alert("No active hazardous waste carboys found in inventory. Please contact the PCO to register a carboy.");
      return;
    }
    
    let carboyOptionsHTML = '';
    snap.forEach(doc => {
      const c = doc.data();
      carboyOptionsHTML += `<option value="${c.containerId}">${c.containerId} - ${c.wasteCode} (${c.currentVolume}L / ${c.capacityLiters}L)</option>`;
    });

    const todayStr = new Date().toISOString().substring(0,10);
    const bodyHTML = `
      <form id="lims-chemical-waste-form" style="display:flex; flex-direction:column; gap:12px; text-align:left;">
        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Select Active Carboy:</label>
          <select id="lims-cw-carboy-id" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; cursor:pointer;">
            ${carboyOptionsHTML}
          </select>
        </div>
        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Deposit Volume (Liters):</label>
          <input type="number" id="lims-cw-volume" step="0.1" required placeholder="e.g. 2.5" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
        </div>
        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Date Spent/Deposited:</label>
          <input type="date" id="lims-cw-date" required value="${todayStr}" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
        </div>
      </form>
    `;
    ...
    const footerHTML = `
      ...
      <button class="settings-btn-primary" onclick="submitLimsChemicalWaste()" style="width:auto; margin:0; padding:10px 20px; background:var(--accent); color:white; font-weight:600;">💾 Save Deposit</button>
    `;

    openActionDrawer("🧪 Log Spent Chemical Waste Deposit", bodyHTML, footerHTML);
  });
}

function submitLimsChemicalWaste() {
  const carboyId = document.getElementById('lims-cw-carboy-id').value;
  const volume = parseFloat(document.getElementById('lims-cw-volume').value);
  const date = document.getElementById('lims-cw-date').value;

  if (!carboyId || isNaN(volume) || volume <= 0 || !date) {
    alert("Please enter a valid chemical deposit volume.");
    return;
  }

  const carboyRef = firestore.collection('pco_inventory').doc(carboyId);
  carboyRef.get().then(doc => {
    if (!doc.exists) return;
    const c = doc.data();
    
    const currentVol = parseFloat(c.currentVolume || 0);
    const capacity = parseFloat(c.capacityLiters || 0);
    const newVol = currentVol + volume;

    if (newVol > capacity) {
      alert(`Error: Adding ${volume}L will exceed the carboy's max capacity of ${capacity}L (currently contains ${currentVol}L). Please select a different container.`);
      return;
    }

    const depositObj = {
      date: date + "T" + new Date().toTimeString().substring(0,8) + "Z",
      volume: volume,
    ...
    carboyRef.update({
      ...
      deposits: firebase.firestore.FieldValue.arrayUnion(depositObj)
    })
    .then(() => {
      alert(`Successfully deposited ${volume}L spent chemicals into carboy ${carboyId}!`);
      closeActionDrawer();
      if (typeof currentMode !== 'undefined' && currentMode === 'lab-spent-chemicals') {
        loadLabSpentChemicalsData();
      }
    })
    .catch(err => alert("Error: " + err.message));
  });
...
function triggerLimsWastewaterForm() {
  const todayStr = new Date().toISOString().substring(0,10);
  const bodyHTML = `
    <form id="lims-wastewater-form" style="display:flex; flex-direction:column; gap:12px; text-align:left;">
      <div style="display:flex; flex-direction:column; gap:4px;">
        <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Lab Sinks Location:</label>
        <input type="text" id="lims-ww-location" required value="CNMS Chemistry General Lab Sinks" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
      </div>
      <div style="display:flex; flex-direction:column; gap:4px;">
        <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">pH Value Measured:</label>
        <input type="number" id="lims-ww-ph" step="0.01" min="0" max="14" required placeholder="e.g. 7.2" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
      </div>
      <div style="display:flex; flex-direction:column; gap:4px;">
        <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Discharged Volume (Liters):</label>
        <input type="number" id="lims-ww-volume" step="0.1" required placeholder="e.g. 40.0" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
      </div>
      <div style="display:flex; gap:10px; align-items:center; margin:6px 0;">
        <input type="checkbox" id="lims-ww-neutralized" onchange="toggleLimsWwNeutralizerSection(this.checked)" style="width:16px; height:16px; cursor:pointer;">
        <label for="lims-ww-neutralized" style="font-size:12px; font-weight:700; color:var(--text-main); cursor:pointer;">Neutralization Treatment done?</label>
      </div>
      <div id="lims-ww-neutralizer-section" style="display:none; flex-direction:column; gap:4px; animation:fadeIn 0.2s ease;">
        <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Neutralizing Agent Used:</label>
        <input type="text" id="lims-ww-agent" placeholder="e.g. Sodium Bicarbonate (NaHCO3)" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
      </div>
      <div style="display:flex; flex-direction:column; gap:4px;">
        <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Date:</label>
        <input type="date" id="lims-ww-date" required value="${todayStr}" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
      </div>
    </form>
  `;
  ...
  const footerHTML = `
    <button class="settings-btn-primary" onclick="closeActionDrawer()" style="width:auto; margin:0; padding:10px 16px; background:rgba(255,255,255,0.06); color:var(--text-muted); border:1px solid var(--border-card);">Cancel</button>
    <button class="settings-btn-primary" onclick="submitLimsWastewater()" style="width:auto; margin:0; padding:10px 20px; background:var(--accent); color:white; font-weight:600;">💾 Save pH Log</button>
  `;

  openActionDrawer("💧 Log Laboratory Effluent pH", bodyHTML, footerHTML);
}

function toggleLimsWwNeutralizerSection(checked) {
  const el = document.getElementById('lims-ww-neutralizer-section');
  if (el) el.style.display = checked ? 'flex' : 'none';
}

function submitLimsWastewater() {
  const loc = document.getElementById('lims-ww-location').value.trim();
  const ph = parseFloat(document.getElementById('lims-ww-ph').value);
  const vol = parseFloat(document.getElementById('lims-ww-volume').value);
  const neutralized = document.getElementById('lims-ww-neutralized').checked;
  const agent = neutralized ? document.getElementById('lims-ww-agent').value.trim() : '';
  const date = document.getElementById('lims-ww-date').value;

  if (!loc || isNaN(ph) || isNaN(vol) || !date) {
    alert("Please fill in all wastewater parameters.");
    return;
  }

  firestore.collection('pco_wastewater').add({
    logId: "WW-LOG-" + Date.now(),
    date: date,
    location: loc,
    phValue: ph,
    neutralizationDone: neutralized,
    neutralizingAgent: agent,
    volumeLiters: vol,
    loggedBy: currentUser.email,
    timestamp: firebase.firestore.FieldValue.serverTimestamp()
  })
  .then(() => {
    alert("Laboratory wastewater pH and effluent log recorded successfully!");
    closeActionDrawer();
    if (typeof currentMode !== 'undefined' && currentMode === 'lab-effluent-ph') {
      loadLabEffluentPhData();
    }
  })
  .catch(err => alert("Error: " + err.message));
}
...
// -------------------------------------------------------------
// PCO LABORATORY DEDICATED VIEW RENDERERS
// -------------------------------------------------------------

function renderLabSpentChemicalsView() {
  const viewport = document.getElementById('viewport-body');
  if (!viewport || !currentUser) return;

  viewport.innerHTML = `
    <div class="home-greeting-card" style="padding: 24px; background: rgba(244,63,94,0.05); border: 1px solid rgba(244,63,94,0.25); border-radius: 20px; text-align: left; margin-bottom: 24px;">
      <h2 style="font-size: 22px; font-weight: 800; font-family: 'Outfit', sans-serif; color: #f43f5e; margin: 0 0 8px 0;">🧪 Spent Chemical Deposits</h2>
      <p style="margin: 0; font-size: 13.5px; color: var(--text-muted);">Record daily spent chemical accumulations and log them directly into active compliance carboys.</p>
    </div>

    <div style="background: var(--bg-card); border: 1px solid var(--border-card); border-radius: 16px; padding: 20px; text-align: left;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
        <h3 style="margin:0; font-size:14px; font-weight:700;">Spent Chemical Logs</h3>
        <button class="settings-btn-primary" onclick="triggerLimsChemicalWasteForm()" style="width:auto; margin:0; padding:8px 16px; font-size:12.5px; background:#f43f5e; color:white; font-weight:600; display:flex; align-items:center; gap:6px; cursor:pointer;">
          <span>🧪</span> Log Spent Chemical Deposit
        </button>
      </div>
      <div style="overflow-x:auto;">
        <table class="gradebook-table" style="width:100%;">
          ...
          <tbody id="lab-spent-chemicals-tbody">
            <tr><td colspan="4" style="text-align:center; padding:20px; color:var(--text-muted);">Loading deposit logs...</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
function loadLabSpentChemicalsData() {
  const tbody = document.getElementById('lab-spent-chemicals-tbody');
  if (!tbody) return;
  ...
  firestore.collection('pco_inventory').get().then(snap => {
    ...
    snap.forEach(doc => {
      const c = doc.data();
      if (c.deposits && c.deposits.length > 0) {
        c.deposits.forEach(d => {
          allDeposits.push({
            carboyId: c.containerId,
            date: d.date,
            volume: d.volume,
            loggedBy: d.loggedBy
          });
    ...
    if (allDeposits.length === 0) {
      tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:20px; color:var(--text-muted);">No spent chemical deposit logs found.</td></tr>`;
      return;
    }

    allDeposits.sort((a, b) => b.date.localeCompare(a.date));

    tbody.innerHTML = allDeposits.map(d => {
      const formattedDate = d.date ? d.date.replace('T', ' ').substring(0, 16) : 'N/A';
      return `
        <tr>
          <td style="font-weight:700; color:var(--text-main);">${formattedDate}</td>
          <td><strong style="color:var(--accent);">${escapeHtml(d.carboyId)}</strong></td>
          <td>${parseFloat(d.volume).toFixed(1)} L</td>
          <td style="font-size:12px; color:var(--text-muted);">${escapeHtml(d.loggedBy)}</td>
        </tr>
      `;
    }).join('');
  }).catch(err => {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:20px; color:var(--incorrect);">Error: ${err.message}</td></tr>`;
  });
}

function renderLabEffluentPhView() {
  const viewport = document.getElementById('viewport-body');
  if (!viewport || !currentUser) return;

  viewport.innerHTML = `
    <div class="home-greeting-card" style="padding: 24px; background: rgba(6,182,212,0.05); border: 1px solid rgba(6,182,212,0.25); border-radius: 20px; text-align: left; margin-bottom: 24px;">
      <h2 style="font-size: 22px; font-weight: 800; font-family: 'Outfit', sans-serif; color: #06b6d4; margin: 0 0 8px 0;">💧 Laboratory Effluent pH Logs</h2>
      <p style="margin: 0; font-size: 13.5px; color: var(--text-muted);">Record daily general lab sink pH neutralizations to ensure compliant university wastewater discharges.</p>
    </div>

    <div style="background: var(--bg-card); border: 1px solid var(--border-card); border-radius: 16px; padding: 20px; text-align: left;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
        <h3 style="margin:0; font-size:14px; font-weight:700;">Wastewater pH Logs</h3>
        <button class="settings-btn-primary" onclick="triggerLimsWastewaterForm()" style="width:auto; margin:0; padding:8px 16px; font-size:12.5px; background:#06b6d4; color:white; font-weight:600; display:flex; align-items:center; gap:6px; cursor:pointer;">
      ...
      <div style="overflow-x:auto;">
        <table class="gradebook-table" style="width:100%;">
          ...
          <tbody id="lab-effluent-ph-tbody">
            <tr><td colspan="7" style="text-align:center; padding:20px; color:var(--text-muted);">Loading pH logs...</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
function loadLabEffluentPhData() {
  const tbody = document.getElementById('lab-effluent-ph-tbody');
  if (!tbody) return;

  firestore.collection('pco_wastewater').get().then(snap => {
    if (snap.empty) {
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:20px; color:var(--text-muted);">No wastewater pH logs found.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = logs.map(l => {
      const phColor = (l.phValue < 6.0 || l.phValue > 9.0) ? 'var(--incorrect)' : 'var(--correct)';
      return `
        <tr>
          <td style="font-weight:700; color:var(--text-main);">${l.date}</td>
          <td>${escapeHtml(l.location)}</td>
          <td style="font-weight:900; color:${phColor}; font-size:14px;">${parseFloat(l.phValue).toFixed(2)}</td>
          <td>${parseFloat(l.volumeLiters).toFixed(1)} L</td>
          <td style="font-weight:700;">${l.neutralizationDone ? '<span style="color:var(--correct);">YES</span>' : '<span style="color:var(--text-muted);">NO</span>'}</td>
          <td>${escapeHtml(l.neutralizingAgent || 'None')}</td>
          <td style="font-size:12px; color:var(--text-muted);">${escapeHtml(l.loggedBy)}</td>
        </tr>
      `;
    }).join('');
  }).catch(err => {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; padding:20px; color:var(--incorrect);">Error: ${err.message}</td></tr>`;
  });
}

function renderLabHazardousInventoryView() {
  const viewport = document.getElementById('viewport-body');
  if (!viewport || !currentUser) return;

  viewport.innerHTML = `
    <div class="home-greeting-card" style="padding: 24px; background: rgba(168,85,247,0.05); border: 1px solid rgba(168,85,247,0.25); border-radius: 20px; text-align: left; margin-bottom: 24px;">
      <h2 style="font-size: 22px; font-weight: 800; font-family: 'Outfit', sans-serif; color: #a855f7; margin: 0 0 8px 0;">📦 Hazardous Waste Inventory</h2>
      <p style="margin: 0; font-size: 13.5px; color: var(--text-muted);">Review chemical carboys currently accumulating inside CNMS Stockroom or transferred to the MRF.</p>
    </div>

    <div style="background: var(--bg-card); border: 1px solid var(--border-card); border-radius: 16px; padding: 20px; text-align: left;">
      <h3 style="margin:0 0 16px 0; font-size:14px; font-weight:700;">Active Accumulation Carboys</h3>
      <div id="lab-inventory-list" class="carboy-grid">
        <div style="font-size:12.5px; color:var(--text-muted); padding:20px; text-align:center; width:100%;">Loading carboys...</div>
      </div>
    </div>
  `;

  loadLabHazardousInventoryData();
}

function loadLabHazardousInventoryData() {
  const container = document.getElementById('lab-inventory-list');
  if (!container) return;

  firestore.collection('pco_inventory').get().then(snap => {
    if (snap.empty) {
      container.innerHTML = `<div class="empty-playlist-msg" style="width:100%;">No chemical carboys found in inventory.</div>`;
      return;
    }
    ...
    snap.forEach(doc => {
      ...
      const currentVol = parseFloat(c.currentVolume || 0);
      const capacity = parseFloat(c.capacityLiters || 0);
      const fillPct = (currentVol / capacity) * 100;
      const started = new Date(c.dateStarted);
      const diffTime = today - started;
      const ageDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
      
      let barClass = '';
      if (ageDays >= 80) barClass = 'alert';
      else if (fillPct >= 90) barClass = 'warning';

      const isStockroom = c.location === 'CNMS Chemistry Stockroom';
      const selectHtml = `
        <select onchange="updateCarboyLocation('${c.containerId}', this.value)" style="padding:6px; border-radius:6px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:11.5px; outline:none; cursor:pointer; width:100%; margin-top:6px;">
          <option value="CNMS Chemistry Stockroom" ${isStockroom ? 'selected' : ''}>Chemistry Stockroom</option>
          <option value="MRF Hazard Wastes Storage" ${!isStockroom ? 'selected' : ''}>MRF Hazard Wastes Storage</option>
        </select>
      `;

      html += `
        <div class="carboy-card" style="background:var(--bg-body); border:1px solid var(--border-card); border-radius:12px; padding:16px; display:flex; flex-direction:column; gap:10px; text-align:left;">
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            ...
            <span style="padding: 4px 8px; border-radius: 6px; font-weight: 700; font-size: 11px; background:rgba(255,255,255,0.05); color:var(--text-muted);">${c.status.toUpperCase()}</span>
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:4px; font-size:12px; color:var(--text-muted);">
            ...
            <div>Vol: <strong style="color:var(--text-main);">${currentVol.toFixed(1)}L / ${capacity}L</strong></div>
            <div>Accumulated: <span style="color:${ageDays >= 80 ? 'var(--pco-alert)' : 'var(--text-main)'}; font-weight:700;">${ageDays} days</span></div>
            <div style="font-size:11px; margin-top:2px;">Location: <strong style="color:var(--text-main);">${escapeHtml(c.location)}</strong></div>
            <div style="margin-top:4px;">
              <label style="font-size:9.5px; font-weight:700; text-transform:uppercase; color:var(--text-muted); display:block; margin-bottom:2px;">Update Storage Facility:</label>
              ${selectHtml}
            </div>
          </div>
        </div>
    ...
    container.innerHTML = `<div style="color:var(--incorrect); padding:20px;">Error loading carboys: ${err.message}</div>`;
  });
}

function updateCarboyLocation(carboyId, newLocation) {
  firestore.collection('pco_inventory').doc(carboyId).update({
    location: newLocation
  }).then(() => {
    showCustomAlert(`Carboy ${carboyId} location marked as "${newLocation}" successfully!`, "success");
    loadLabHazardousInventoryData();
  }).catch(err => {
...
function renderLabSpillIncidentView() {
  const viewport = document.getElementById('viewport-body');
  if (!viewport || !currentUser) return;
  ...
  viewport.innerHTML = `
    <div class="home-greeting-card" style="padding: 24px; background: rgba(239,68,68,0.05); border: 1px solid rgba(239,68,68,0.25); border-radius: 20px; text-align: left; margin-bottom: 24px;">
      <h2 style="font-size: 22px; font-weight: 800; font-family: 'Outfit', sans-serif; color: #ef4444; margin: 0 0 8px 0;">🚨 Spill Incident Notification</h2>
      <p style="margin: 0; font-size: 13.5px; color: var(--text-muted);">Quickly report chemical spill incidents to the Pollution Control Officer (PCO) for response and recordkeeping.</p>
    </div>

    <div style="background: var(--bg-card); border: 1px solid var(--border-card); border-radius: 16px; padding: 24px; text-align: left; max-width: 600px; margin: 0 auto;">
      <h3 style="margin: 0 0 16px 0; font-size: 16px; font-weight: 700; font-family: 'Outfit', sans-serif; border-bottom: 1px dashed var(--border-card); padding-bottom:10px;">📋 Report Spill Incident</h3>
      
      <form id="lab-spill-incident-form" onsubmit="submitLabSpillIncident(event)" style="display:flex; flex-direction:column; gap:14px;">
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Spill Date:</label>
            <input type="date" id="spill-date" required value="${todayStr}" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
          </div>
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Spill Time:</label>
            <input type="time" id="spill-time" required value="${timeStr}" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
          </div>
        </div>

        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Incident Location:</label>
          <input type="text" id="spill-location" required value="CNSM Chemistry Stockroom Area" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
        </div>

        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Substance Spilled:</label>
          <input type="text" id="spill-substance" required placeholder="e.g. Spent Solvents (G703 / G704)" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
        </div>

        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Estimated Volume (Liters):</label>
          <input type="number" id="spill-volume" step="0.1" required placeholder="e.g. 2.5" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
        </div>

        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Incident Description:</label>
          <textarea id="spill-description" required rows="3" placeholder="Describe how the spill occurred..." style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; resize:vertical; font-family:sans-serif;"></textarea>
        </div>

        <div style="display:flex; flex-direction:column; gap:4px;">
          <label style="font-size:11px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Response/Corrective Action Taken:</label>
          <textarea id="spill-action" required rows="3" placeholder="Describe the immediate containment/cleanup steps taken..." style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; resize:vertical; font-family:sans-serif;"></textarea>
        </div>

        <button type="submit" class="settings-btn-primary" style="margin-top:8px; background:#ef4444; color:white; font-weight:700; font-size:13.5px; padding:12px; cursor:pointer;">📤 Submit Spill Notification</button>
      </form>
    </div>
  `;
}

function submitLabSpillIncident(event) {
  event.preventDefault();

  const date = document.getElementById('spill-date').value;
  const time = document.getElementById('spill-time').value;
  const loc = document.getElementById('spill-location').value.trim();
  const substance = document.getElementById('spill-substance').value.trim();
  const vol = parseFloat(document.getElementById('spill-volume').value);
  const desc = document.getElementById('spill-description').value.trim();
  const action = document.getElementById('spill-action').value.trim();

  if (!date || !time || !loc || !substance || isNaN(vol) || !desc || !action) {
    showCustomAlert("Please fill in all incident parameters.", "error");
    return;
  }
  ...
  const incidentData = {
    ...
    status: 'reported',
    embNotified24h: false,
    loggedBy: currentUser.email,
    timestamp: firebase.firestore.FieldValue.serverTimestamp()
  };

  firestore.collection('pco_incidents').add(incidentData).then(() => {
    showCustomAlert("Spill Incident Notification submitted successfully to the PCO!", "success");
    document.getElementById('lab-spill-incident-form').reset();
  }).catch(err => {
    showCustomAlert("Submission failed: " + err.message, "error");
  });
}

// Bind to window for global routing
window.triggerLimsChemicalWasteForm = triggerLimsChemicalWasteForm;
window.submitLimsChemicalWaste = submitLimsChemicalWaste;
window.triggerLimsWastewaterForm = triggerLimsWastewaterForm;
window.submitLimsWastewater = submitLimsWastewater;

window.renderLabSpentChemicalsView = renderLabSpentChemicalsView;
window.renderLabEffluentPhView = renderLabEffluentPhView;
window.renderLabHazardousInventoryView = renderLabHazardousInventoryView;
window.renderLabSpillIncidentView = renderLabSpillIncidentView;
window.updateCarboyLocation = updateCarboyLocation;
window.submitLabSpillIncident = submitLabSpillIncident;
window.loadLabSpentChemicalsData = loadLabSpentChemicalsData;
window.loadLabEffluentPhData = loadLabEffluentPhData;
window.loadLabHazardousInventoryData = loadLabHazardousInventoryData;
