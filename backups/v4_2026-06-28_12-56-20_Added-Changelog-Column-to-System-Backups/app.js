...
function determineUserRole(email) {
  ...
  if (lowerEmail === 'pco@msugensan.edu.ph') {
    return 'pco';
  }
  // Faculty: ramon.eduque@msugensan.edu.ph
  if (encoded === 'cmFtb24uZWR1cXVlQG1zdWdlbnNhbi5lZHUucGg=' || lowerEmail === 'faculty@msugensan.edu.ph') {
    return 'faculty';
...
function sanitizeUserRoles(user) {
  ...
  // Assign PCO role to Ramon M. Eduque, Jr. and pco@msugensan.edu.ph
  if (email === 'ramon.eduque@msugensan.edu.ph' || email === 'pco@msugensan.edu.ph') {
    if (!user.roles.includes('pco')) {
      user.roles.push('pco');
    }
  }
  ...
  if (user.role === 'admin' || user.roles.includes('admin')) {
    user.roles = ['admin', 'faculty', 'laboratory', 'pco', 'student'];
  }
...
function loadOrCreateUserProfile(firebaseUser) {
  ...
  firestore.collection("students").doc(email).get()
    .then((doc) => {
      const isPcoRoleName = (r) => ['pco', 'pco_head', 'pco_laboratory', 'pco_office', 'pco_college'].includes(r);
      ...
      if (doc.exists) {
        ...
        if (currentUserRole === 'unassigned') {
          // Unassigned accounts go straight to welcome dashboard, skip onboarding overlay
          ...
          setMode('welcome');
        } else if (currentUserRole === 'student' && (!currentUser.name || !currentUser.studentId || !currentUser.subjects || currentUser.subjects.length === 0)) {
          // Student needs onboarding
          showOnboardingStage(2);
        ...
        if (localProfile) {
          ...
          if (currentUserRole === 'admin' || currentUserRole === 'faculty' || currentUserRole === 'laboratory' || isFreshPco) {
            ...
            // New unassigned user landing page flow
            currentUser = {
              name: firebaseUser.displayName || email.split('@')[0],
              email: email,
              studentId: '',
...
function updateProfileUI() {
  ...
  if (currentUserRole === 'admin') {
    ...
    document.getElementById('header-title').innerText = 'PCO Environmental';
    ...
    document.getElementById('header-subtitle').innerText = 'Welcome guest';
    document.body.classList.remove('pco-active-role');
  } else {
    document.getElementById('header-icon').innerText = '🧪';
...
function renderRoleSwitcher() {
  ...
  if (roles.includes('admin')) {
    container.innerHTML = `
      ...
      <button class="role-switch-btn chairperson" disabled title="Chairperson Executive Center (Planned)">
        🏛️ Chairperson
      </button>
      ...
      <button onclick="switchActiveRole('pco')" 
              class="role-switch-btn pco ${currentUserRole === 'pco' ? 'active' : ''}" 
              title="Switch profile to PCO/EMIS">
        🌿 PCO/EMIS
      </button>
  ...
  if (currentUserRole === 'unassigned') {
    ...
    html += `
      ...
      <div class="mode-tabs">
        ...
        <button class="mode-tab-btn" id="tab-chairperson" onclick="setMode('chairperson')">🏛️ Chairperson Center</button>
      </div>
    `;
  } else if (currentUserRole === 'pco') {
    ...
    if (subrole === 'pco_head' || subrole === 'pco_laboratory' || subrole === 'pco_office' || subrole === 'pco_college' || subrole === 'pco') {
      html += `<button class="mode-tab-btn active" id="tab-pco-dashboard" onclick="setMode('pco-dashboard')">🏠 Dashboard</button>`;
    }
    
    if (subrole === 'pco_head' || subrole === 'pco_laboratory' || subrole === 'pco') {
      html += `<button class="mode-tab-btn" id="tab-pco-waste" onclick="setMode('pco-waste')">📋 Waste Ledger</button>`;
    }
    
    if (subrole === 'pco_head' || subrole === 'pco_laboratory' || subrole === 'pco_office' || subrole === 'pco') {
      html += `<button class="mode-tab-btn" id="tab-pco-generators" onclick="setMode('pco-generators')">⚡ Air Emissions</button>`;
    }
    
    if (subrole === 'pco_head' || subrole === 'pco_office' || subrole === 'pco') {
    ...
    if (subrole === 'pco_head' || subrole === 'pco_laboratory' || subrole === 'pco_college' || subrole === 'pco') {
  ...
  if (currentUserRole === 'admin') {
    viewport.innerHTML = `
      ...
      <div style="background: var(--bg-card); border: 1px solid var(--border-card); padding: 20px; border-radius: 14px; text-align: left; margin-bottom: 24px;">
        ...
        <p style="margin: 0 0 16px 0; font-size: 13px; color: var(--text-muted);">Manage version control restore states of core portal code and configurations. Up to 10 versions are kept. Note: Backups are triggered locally via the CLI utility.</p>
  ...
  firestore.collection('students')
    ...
    .then(querySnapshot => {
      ...
      usersList.forEach(userData => {
        ...
        const acadRoles = ['faculty', 'laboratory', 'admin'];
        acadRoles.forEach(r => {
        ...
        const pcoRoles = ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college'];
        pcoRoles.forEach(r => {
          ...
          <table style="width:100%; border-collapse:collapse; font-size:13px; text-align:left; min-width: 500px;">
            <thead>
              <tr style="border-bottom:1px dashed var(--border-card); color:var(--text-muted); font-size:11px; text-transform:uppercase;">
                ...
                <th style="padding:8px 4px;">Change Description</th>
      ...
      backups.forEach(b => {
        html += `
          <tr style="border-bottom:1px solid rgba(255,255,255,0.02);">
            ...
            <td style="padding:8px 4px; color:var(--text-muted); font-style:italic;">${escapeHtml(b.changelog || 'Manual version update')}</td>
...
function renderWelcomeDashboard() {
  ...
  viewport.innerHTML = `
    ...
    <div class="welcome-hero-banner">
      <div style="font-size: 11px; font-weight:700; color:var(--accent); text-transform:uppercase; margin-bottom:8px; display:inline-block; background:rgba(59,130,246,0.15); padding:4px 8px; border-radius:6px; font-family:'Outfit',sans-serif;">MSU GenSan — CNMS</div>
      <h1 class="welcome-hero-title">Department of Chemistry Portal</h1>
    ...
    <div class="welcome-modules-grid">
      ...
      <div class="welcome-module-card" onclick="handleWelcomeModuleClick('pco')">
        <div class="welcome-module-icon">🌿</div>
        <h4 class="welcome-module-title">PCO Environmental</h4>
        <p class="welcome-module-desc">Monitor generator runtimes, hazardous inventories, wastewater pH, and compile SMR reports.</p>
      </div>

      <!-- Chairperson Mirror Card (Locked Placeholder) -->
      <div class="welcome-module-card locked" onclick="handleWelcomeModuleClick('chairperson')">
        ...
        <p class="welcome-module-desc">Carbon footprint analytics, real-time chemical tracking, and student clearances overview (Next Phase).</p>
      </div>
    </div>
    ...
    <div class="welcome-sections-layout">
      <!-- Left Column: News and Announcements -->
  ...
  if (currentUserRole === 'admin' || currentUser.role === 'chairperson') {
...
function handleWelcomeModuleClick(module) {
  if (module === 'chairperson') {
    if (currentUser.role === 'chairperson' || currentUserRole === 'admin') {
      setMode('chairperson');
    } else {
      showCustomAlert("Access Locked: The Chairperson Executive Center is scheduled for Phase 3 integration.", "info");
    }
    return;
  }
  ...
  if (currentUserRole === 'admin') {
    ...
    if (module === 'pco') {
      hasRole = ['pco', 'pco_head', 'pco_laboratory', 'pco_office', 'pco_college'].some(r => userRoles.includes(r));
    } else {
  ...
  if (hasRole) {
    ...
    showCustomAlert(`Access Denied: Your account does not have the '${module.toUpperCase()}' role. Please apply for this role using the application form.`, "error");
  }
}
...
function loadPortalContent() {
  firestore.collection('portal_content').get().then(snap => {
    let items = [];
    if (snap.empty) {
      // Seed mockup data so it's not blank
      items = [
        {
          type: 'announcement',
      ...
      // Batch write to database as initial seed
      items.forEach(item => {
        firestore.collection('portal_content').add(item);
      });
    } else {
      snap.forEach(doc => {
...
function renderPortalRoleApplicationForm() {
  ...
  firestore.collection('role_applications').where('email', '==', currentUser.email).get().then(snap => {
    ...
    // If no pending application, display the form
    container.innerHTML = `
      <div class="app-role-form-container">
        ...
        <form id="portal-role-app-form" onsubmit="handleRoleApplicationSubmit(event)" style="display:flex; flex-direction:column; gap:12px;">
          <div style="display:flex; flex-direction:column; gap:4px;">
            ...
            <input type="text" id="role-app-name" required placeholder="e.g. Juan Dela Cruz" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
          </div>
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Email Address:</label>
            <input type="email" id="role-app-email" required value="${currentUser && currentUser.email && currentUser.email.endsWith('@msugensan.edu.ph') ? currentUser.email : 'juan.delacruz@msugensan.edu.ph'}" placeholder="juan.delacruz@msugensan.edu.ph" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; font-family:monospace;">
          </div>
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Office / Room Location:</label>
            <input type="text" id="role-app-office" required placeholder="Department of Chemistry, RSRC Bldg." style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
          </div>
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Target Portal Role:</label>
            <select id="role-app-target-role" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; cursor:pointer;">
...
function handleRoleApplicationSubmit(event) {
  ...
  const name = document.getElementById('role-app-name').value.trim();
  const email = document.getElementById('role-app-email').value.trim();
  const office = document.getElementById('role-app-office').value.trim();
  const targetRole = document.getElementById('role-app-target-role').value;
  const idFile = document.getElementById('role-app-univ-id').files[0];
  ...
  const appData = {
    ...
    univIdBase64: window.lastUploadedUnivIdBase64 || '',
    status: 'pending',
    timestamp: firebase.firestore.FieldValue.serverTimestamp()
  };
...
function renderAdminRoleApplications() {
  const viewport = document.getElementById('viewport-body');
  if (!viewport) return;
