...
function renderRoleSwitcher() {
  ...
  if (roles.includes('chairperson') && roles.includes('faculty')) {
    container.innerHTML = `
    ...
    container.innerHTML = `
      ...
      <button onclick="switchActiveRole('pco')" 
              ...
              title="Switch profile to PCO/EIS">
        🌿 PCO/EIS
      </button>
    ...
    .then(backups => {
      ...
      let html = `
        <div style="display:flex; flex-direction:column; gap:10px; overflow-x:auto;">
          <table style="width:100%; border-collapse:collapse; font-size:13px; text-align:left; min-width: 450px;">
            <thead>
...
function handleWelcomeModuleClick(module) {
  ...
  if (hasRole) {
    ...
    showCustomAlert(`Access Restricted: Your account does not have the '${module.toUpperCase()}' role. Please apply for this role using the application access request form below.`, "error");
  }
}
