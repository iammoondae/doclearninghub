...
if (!document.querySelector('script[src*="chairperson.js"]')) {
  ...
  const script = document.createElement('script');
  script.src = 'chairperson.js?v=' + Date.now();
  document.head.appendChild(script);
}
...
function determineUserRole(email) {
  ...
  if (encoded === 'cmFtb24uZWR1cXVlQG1zdWdlbnNhbi5lZHUucGg=' || lowerEmail === 'faculty@msugensan.edu.ph') {
    return 'faculty';
  // Admin: admin@msugensan.edu.ph
  } else if (lowerEmail === 'admin@msugensan.edu.ph') {
    return 'admin';
  // Laboratory stockroom account default debug emails
  } else if (lowerEmail === 'laboratory@msugensan.edu.ph' || lowerEmail === 'lab@msugensan.edu.ph') {
    return 'laboratory';
  }
  return 'student';
}
...
function sanitizeUserRoles(user) {
  ...
  if (!isAdmin && user.roles.includes('pco')) {
    const hasSpecificPcoRole = user.roles.some(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college'].includes(r));
    if (hasSpecificPcoRole) {
      user.roles = user.roles.filter(r => r !== 'pco');
      if (user.role === 'pco') {
        const firstSpecific = user.roles.find(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college'].includes(r));
  ...
  if (user.role === 'admin' || user.roles.includes('admin')) {
    user.roles = ['admin', 'faculty', 'laboratory', 'pco_head', 'student', 'chairperson'];
  }
  ...
  firestore.collection('students').get().then(snapshot => {
    snapshot.forEach(doc => {
      ...
      if (!isAdmin && updatedRoles.includes('pco')) {
        const hasSpecificPcoRole = updatedRoles.some(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college'].includes(r));
        if (hasSpecificPcoRole) {
          updatedRoles = updatedRoles.filter(r => r !== 'pco');
          if (updatedRole === 'pco') {
            const firstSpecific = updatedRoles.find(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college'].includes(r));
...
function updateProfileUI() {
  ...
  // Hide theme switcher button for Admin accounts
  const themeBtn = document.getElementById('theme-btn');
  if (themeBtn) {
    const isAdminAccount = currentUser && (currentUser.role === 'admin' || currentUserRole === 'admin' || (currentUser.roles && currentUser.roles.includes('admin')));
    themeBtn.style.display = isAdminAccount ? 'none' : '';
  }
...
function renderSidebarNavigation() {
  ...
  if (currentUserRole === 'unassigned') {
    ...
    if (subrole === 'pco_head' || subrole === 'pco_laboratory' || subrole === 'pco_college') {
      ...
      <div class="app-role-form-container">
        ...
        <form id="portal-role-app-form" onsubmit="handleRoleApplicationSubmit(event)" style="display:flex; flex-direction:column; gap:12px;">
          ...
          <div style="display:flex; flex-direction:column; gap:4px;">
            ...
            <input type="email" id="role-app-email" required value="${currentUser && currentUser.email ? currentUser.email : ''}" placeholder="e.g. juan.delacruz@msugensan.edu.ph" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; font-family:monospace;" ${currentUser && currentUser.email ? 'readonly style="opacity: 0.6; cursor: not-allowed;"' : ''}>
          </div>
          <div style="display:flex; flex-direction:column; gap:4px;">
