...
function submitLimsChemicalWaste() {
  ...
  carboyRef.get().then(doc => {
    ...
    const newVol = c.currentVolume + volume;
    if (newVol > c.capacityLiters) {
      alert(`Error: Adding ${volume}L will exceed the carboy's max capacity of ${c.capacityLiters}L (currently contains ${c.currentVolume}L). Please select a different container.`);
      return;
    }
...
window.toggleLimsWwNeutralizerSection = toggleLimsWwNeutralizerSection;
