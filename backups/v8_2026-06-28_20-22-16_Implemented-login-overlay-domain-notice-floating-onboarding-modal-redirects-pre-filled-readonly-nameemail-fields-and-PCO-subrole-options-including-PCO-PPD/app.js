...
function sanitizeUserRoles(user) {
  ...
  if (!isAdmin && user.roles.includes('pco')) {
    const hasSpecificPcoRole = user.roles.some(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'].includes(r));
    if (hasSpecificPcoRole) {
      user.roles = user.roles.filter(r => r !== 'pco');
      if (user.role === 'pco') {
        const firstSpecific = user.roles.find(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'].includes(r));
  ...
  firestore.collection('students').get().then(snapshot => {
    snapshot.forEach(doc => {
      ...
      if (!isAdmin && updatedRoles.includes('pco')) {
        const hasSpecificPcoRole = updatedRoles.some(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'].includes(r));
        if (hasSpecificPcoRole) {
          updatedRoles = updatedRoles.filter(r => r !== 'pco');
          if (updatedRole === 'pco') {
            const firstSpecific = updatedRoles.find(r => ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'].includes(r));
...
function loadOrCreateUserProfile(firebaseUser) {
  ...
  firestore.collection("students").doc(email).get()
    .then((doc) => {
      const isPcoRoleName = (r) => ['pco', 'pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'].includes(r);
...
function renderSidebarNavigation() {
  ...
  if (currentUserRole === 'unassigned') {
    ...
    if (subrole === 'pco_head' || subrole === 'pco_laboratory' || subrole === 'pco_office' || subrole === 'pco_college' || subrole === 'pco_ppd') {
  ...
  firestore.collection('students')
    ...
    .then(querySnapshot => {
      ...
      usersList.forEach(userData => {
        ...
        const pcoRoles = ['pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'];
        pcoRoles.forEach(r => {
...
function handleWelcomeModuleClick(module) {
  ...
  if (currentUserRole === 'admin') {
    ...
    if (module === 'pco') {
      hasRole = ['pco', 'pco_head', 'pco_laboratory', 'pco_office', 'pco_college', 'pco_ppd'].some(r => userRoles.includes(r));
    } else {
...
function handleRoleApplicationSubmit(event) {
  ...
  const office = document.getElementById('role-app-office' + suffix).value.trim();
    ...
    if (pendingDoc) {
      ...
      body.innerHTML = `
        <form id="portal-role-app-form-modal" onsubmit="handleRoleApplicationSubmit(event)" style="display:flex; flex-direction:column; gap:12px;">
          ...
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Office / Room Location:</label>
            <input type="text" id="role-app-office-modal" required placeholder="Department of Chemistry, RSRC Bldg." style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none;">
          </div>
          <div style="display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase;">Target Portal Role:</label>
            <select id="role-app-target-role-modal" style="padding:10px; border-radius:8px; border:1px solid var(--border-card); background:var(--bg-body); color:var(--text-main); font-size:13px; outline:none; cursor:pointer;">
              ${isPcoModule ? `
